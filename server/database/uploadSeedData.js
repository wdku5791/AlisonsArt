  ...
  .then(() => {
    return db.tx((t) => {
      return t.batch(auctions.map(auction => (t.query('INSERT INTO auctions \
                ...
                VALUES \
                ((SELECT id FROM users WHERE id = ${owner_id}), (SELECT id FROM artworks WHERE id = ${artwork_id}), ${start_date}, ${end_date}, ${start_price}, ${buyout_price}, ${current_bid_id}, ${current_bid}, ${bid_counter})\
